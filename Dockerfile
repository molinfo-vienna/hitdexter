...
RUN apk update && apk add git libxrender libxext-dev libxau g++ wget entr
...
RUN --mount=type=cache,target=/root/.cache/pip \
    micromamba run -n hitdexter pip install .

RUN --mount=type=cache,target=/root/.cache/pip \
    micromamba run -n hitdexter pip install nerdd-link==0.2.13
...
ENTRYPOINT micromamba run -n hitdexter \
